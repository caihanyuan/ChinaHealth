package com.chinahelth.support.lib;
...
public abstract class MyAsyncTask<Params, Progress, Result> {
    ...
    private static final Executor CANCEL_OPERATION_THREAD_POOL_EXECUTOR
                ...
                public void rejectedExecution(Runnable r, ThreadPoolExecutor e) {
                    if (!e.isShutdown()) {
                        ...
                        if (runnable instanceof FutureTask) {
                            ...
                            CANCEL_OPERATION_THREAD_POOL_EXECUTOR.execute(futureTask);
                        }
                        e.execute(r);
                    }
                }
    ...
    private static final Executor IO_THREAD_POOL_EXECUTOR
            ...
            new ThreadPoolExecutor.DiscardOldestPolicy());
    private static final ThreadFactory sDatabaseThreadFactory = new ThreadFactory() {
        private final AtomicInteger mCount = new AtomicInteger(1);

        public Thread newThread(Runnable r) {
            return new Thread(r, "AsyncTask Database Access #" + mCount.getAndIncrement());
        }
    };
    private static final Executor DATABASE_ACCESS_THREAD_POOL_EXECUTOR
            = new ThreadPoolExecutor(6, 10, KEEP_ALIVE,
            TimeUnit.SECONDS, new LinkedBlockingDeque<Runnable>(50), sDatabaseThreadFactory,
            new ThreadPoolExecutor.DiscardOldestPolicy() {
                @Override
                public void rejectedExecution(Runnable r, ThreadPoolExecutor e) {
    ...
    public void executeOnDatabase(Params... params) {
        executeOnExecutor(DATABASE_ACCESS_THREAD_POOL_EXECUTOR, params);
    }
    ...
    private void finish(Result result) {
        LogUtils.d(LOG_TAG, "AsyncTask finish");
        if (isCancelled()) {
            onCancelled(result);
        } else {
